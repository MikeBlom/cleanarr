...
from .models import ConversionJob, ConversionRequest, JobStatus, RequestStatus
...
def _write_config_files(
    db, req: ConversionRequest | None = None
) -> tuple[str, str, str]:
    ...
    prof = {
        "words": words,
        "phrases": phrases,
        "padding_ms": req.profanity_padding_ms
        if req and req.profanity_padding_ms is not None
        else int(app_settings.get(db, "profanity_padding_ms")),
        "whisper_model": req.whisper_model
        if req and req.whisper_model
        else app_settings.get(db, "whisper_model"),
    }
    ...
    nud = {
        ...
        "confidence_threshold": req.nudity_confidence
        if req and req.nudity_confidence is not None
        else float(app_settings.get(db, "nudity_confidence")),
        "sample_fps": req.nudity_sample_fps
        if req and req.nudity_sample_fps is not None
        else float(app_settings.get(db, "nudity_sample_fps")),
        "padding_ms": req.nudity_padding_ms
        if req and req.nudity_padding_ms is not None
        else int(app_settings.get(db, "nudity_padding_ms")),
        "scene_merge_gap_ms": req.nudity_scene_merge_gap_ms
        if req and req.nudity_scene_merge_gap_ms is not None
        else int(app_settings.get(db, "nudity_scene_merge_gap_ms")),
        "frame_width": 640,
        # Multi-model pipeline settings
        "detectors": _json.loads(req.nudity_detectors_json)
        if req and req.nudity_detectors_json
        else _json.loads(app_settings.get(db, "nudity_detectors")),
        "ensemble_strategy": req.nudity_ensemble_strategy
        if req and req.nudity_ensemble_strategy
        else app_settings.get(db, "nudity_ensemble_strategy"),
        "temporal_enabled": (
            req.nudity_temporal_enabled
            if req and req.nudity_temporal_enabled is not None
            else app_settings.get(db, "nudity_temporal_enabled").lower() == "true"
        ),
        "temporal_window": req.nudity_temporal_window
        if req and req.nudity_temporal_window is not None
        else int(app_settings.get(db, "nudity_temporal_window")),
        "temporal_min_flagged": req.nudity_temporal_min_flagged
        if req and req.nudity_temporal_min_flagged is not None
        else int(app_settings.get(db, "nudity_temporal_min_flagged")),
        "extraction_mode": req.nudity_extraction_mode
        if req and req.nudity_extraction_mode
        else app_settings.get(db, "nudity_extraction_mode"),
        "device": app_settings.get(db, "nudity_device"),
    }
    prof_path = tempfile.NamedTemporaryFile(
        mode="w", suffix=".json", prefix="cleanarr_prof_", delete=False
    )
    prof_path.write(_json.dumps(prof))
    prof_path.close()
    nud_path = tempfile.NamedTemporaryFile(
        mode="w", suffix=".json", prefix="cleanarr_nud_", delete=False
    )
    ...
    viol = {
        ...
        "confidence_threshold": req.violence_confidence
        if req and req.violence_confidence is not None
        else float(app_settings.get(db, "violence_confidence")),
        "sample_fps": req.violence_sample_fps
        if req and req.violence_sample_fps is not None
        else float(app_settings.get(db, "violence_sample_fps")),
        "padding_ms": req.violence_padding_ms
        if req and req.violence_padding_ms is not None
        else int(app_settings.get(db, "violence_padding_ms")),
        "scene_merge_gap_ms": req.violence_scene_merge_gap_ms
        if req and req.violence_scene_merge_gap_ms is not None
        else int(app_settings.get(db, "violence_scene_merge_gap_ms")),
        "frame_width": 640,
        "detectors": _json.loads(req.violence_detectors_json)
        if req and req.violence_detectors_json
        else _json.loads(app_settings.get(db, "violence_detectors")),
        "ensemble_strategy": req.violence_ensemble_strategy
        if req and req.violence_ensemble_strategy
        else app_settings.get(db, "violence_ensemble_strategy"),
        "temporal_enabled": (
            req.violence_temporal_enabled
            if req and req.violence_temporal_enabled is not None
            else app_settings.get(db, "violence_temporal_enabled").lower() == "true"
        ),
        "temporal_window": req.violence_temporal_window
        if req and req.violence_temporal_window is not None
        else int(app_settings.get(db, "violence_temporal_window")),
        "temporal_min_flagged": req.violence_temporal_min_flagged
        if req and req.violence_temporal_min_flagged is not None
        else int(app_settings.get(db, "violence_temporal_min_flagged")),
        "extraction_mode": req.violence_extraction_mode
        if req and req.violence_extraction_mode
        else app_settings.get(db, "violence_extraction_mode"),
        "device": app_settings.get(db, "violence_device"),
    }
    viol_path = tempfile.NamedTemporaryFile(
        mode="w", suffix=".json", prefix="cleanarr_viol_", delete=False
    )
        ...
        entries.append(
            {
                ...
                "type": "nudity",
                "description": ", ".join(
                    _NUDITY_LABELS.get(lbl, lbl) for lbl in labels
                ),
            }
        )
        ...
        entries.append(
            {
                ...
                "type": "violence",
                "description": ", ".join(
                    _VIOLENCE_LABELS.get(lbl, lbl) for lbl in labels
                ),
            }
        )
...
def _update_progress(
    line: str, progress: dict, captured_intervals: dict | None = None
) -> bool:
    ...
    if captured_intervals is not None:
        ...
        if m:
            ...
            bucket = (
                "intervals"
                if phase == "profanity"
                else "nudity_intervals"
                if phase == "nudity"
                else "violence_intervals"
                if phase == "violence"
                else None
            )
            if bucket:
    ...
    if "--- Profanity scan ---" in line:
        ...
        if (
            len(parts) == 2
            and parts[0].isdigit()
            and progress["nudity"]["status"] != "complete"
        ):
        ...
        if (
            ...
            analysis_pct = (
                int(int(parts[0]) / int(parts[1]) * 50) if int(parts[1]) > 0 else 0
            )
        ...
        if (
            len(parts) == 2
            and parts[0].isdigit()
            and progress["violence"]["status"] != "complete"
        ):
            new_pct = int(parts[0]) // 2
            progress["violence"]["pct"] = max(
                progress["violence"].get("pct", 0), new_pct
            )
...
def _cancel_watcher(
    job_id: int, proc: subprocess.Popen, stop_event: threading.Event
) -> None:
...
def _run_job(job_id: int) -> None:
    ...
    try:
        ...
        if can_resume:
            print(
                f"  Resuming job {job_id} — sidecar found from previous run", flush=True
            )
        ...
        try:
            ...
            for line in proc.stdout:  # type: ignore[union-attr]
                ...
                is_tqdm = "\r" in clean or (
                    "|" in clean
                    and "%" in clean
                    and ("frames/s" in clean or "it/s" in clean)
                )
                if is_tqdm:
            ...
            if not sidecar_path.exists():
                try:
                    backup = {
                        ...
                        "nudity_intervals": captured_intervals.get(
                            "nudity_intervals", []
                        ),
                        "nudity_source": "",
                        "violence_intervals": captured_intervals.get(
                            "violence_intervals", []
                        ),
                        "violence_source": "",
                    }
...
def _recover_stale_jobs() -> None:
    ...
    try:
        stale = (
            ...
            .filter(ConversionJob.status == JobStatus.running)
            .all()
        )
        for job in stale:
        ...
        if stale:
            db.commit()
            print(
                f"Recovered {len(stale)} stale running job(s) back to queued.",
                flush=True,
            )
    finally:
        db.close()
        ...
        try:
            ...
            try:
                job = (
                    ...
                    .order_by(
                        ConversionJob.priority.asc(), ConversionJob.created_at.asc()
                    )
                    .limit(1)
                    .first()
                )
