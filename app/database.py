...
def _migrate() -> None:
    with engine.connect() as conn:
        ...
        if "system_task_runs" in [r[0] for r in conn.execute(text("SELECT name FROM sqlite_master WHERE type='table'")).fetchall()]:
            ...
            if "progress_current" not in st_cols:
                conn.execute(text("ALTER TABLE system_task_runs ADD COLUMN progress_current INTEGER"))
            if "progress_total" not in st_cols:
                conn.execute(text("ALTER TABLE system_task_runs ADD COLUMN progress_total INTEGER"))
        conn.commit()
