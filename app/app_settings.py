...
_DEFAULTS: dict[str, tuple[str, str]] = {
    ...
    "nudity_scene_merge_gap_ms": (
        "5000",
        "Bridge nudity detections within this gap (ms) into one continuous blackout",
    ),
    # Multi-model nudity pipeline
    "nudity_detectors": (
        '["nudenet"]',
        "JSON list of detector backends to use (nudenet, vit_nsfw)",
    ),
    "nudity_ensemble_strategy": (
        "weighted_average",
        "Ensemble voting strategy (weighted_average, unanimous, any)",
    ),
    "nudity_temporal_enabled": (
        "false",
        "Enable temporal consistency filter to suppress isolated false positives",
    ),
    "nudity_temporal_window": (
        "5",
        "Sliding window size for temporal filter (number of frames)",
    ),
    "nudity_temporal_min_flagged": (
        "3",
        "Minimum flagged frames in window to keep a detection",
    ),
    "nudity_extraction_mode": (
        "fixed_fps",
        "Frame extraction mode (fixed_fps, keyframe, auto)",
    ),
    ...
    "violence_scene_merge_gap_ms": (
        "5000",
        "Bridge violence detections within this gap (ms)",
    ),
    "violence_detectors": (
        '["siglip_violence"]',
        "JSON list of violence detector backends (siglip_violence, vit_violence)",
    ),
    "violence_ensemble_strategy": (
        "weighted_average",
        "Violence ensemble voting strategy",
    ),
    "violence_temporal_enabled": (
        "false",
        "Enable temporal consistency filter for violence detection",
    ),
    "violence_temporal_window": (
        "5",
        "Sliding window size for violence temporal filter",
    ),
    "violence_temporal_min_flagged": (
        "3",
        "Minimum flagged frames in window for violence",
    ),
    "violence_extraction_mode": (
        "fixed_fps",
        "Frame extraction mode for violence (fixed_fps, keyframe, auto)",
    ),
    "violence_device": ("auto", "Device for violence ML models (auto, cpu, cuda)"),
    # AI Content Advisor (Ollama)
    "ollama_url": (
        "http://localhost:11434",
        "Ollama API URL for AI content evaluation",
    ),
